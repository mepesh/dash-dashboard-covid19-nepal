...
# changing df values parsa and nawalpur

dis_df.at[74, "dis_en"] = "Nawalpur"
dis_df.at[75, "dis_en"] = "Parasi"
...
def country_wise_summary(country):
    ...
    if country == "Total":
        detail = country_df.sum(axis=0, skipna=True).to_frame(name="sum")
        data = {
            "Confirmed": [detail.loc["Confirmed", "sum"]],
            "Recovered": [detail.loc["Recovered", "sum"]],
            "Deaths": [detail.loc["Deaths", "sum"]],
        }
...
def provience_test_all(provience):
    rdt_total = pd.read_csv(
        "https://raw.githubusercontent.com/mepesh/python-dashboard-covid19/master/site-report-rdt-total.csv"
    )
    rdt = pd.read_csv(
    ...
    rsum = rdt_total.iloc[-1, -1]
    print(rsum)
    r = rdt.iloc[:, -1]
    i = isolation.iloc[:, -1]
    q = quaratine.iloc[:, -1]
    if provience == 8:
        ret = [rsum, q.sum(), i.sum(), isolation.columns[-1]]
    else:
...
df = get_ac_re_type("municipality")
df = df.rename(
    columns={"Active Cases": "activecases", "Recovered Cases": "recoveredcases"}
)
...
sidebar_header = dbc.Row(
    [
        dbc.Col(md=1),
        dbc.Col(html.H5("Covid 19 | Dashboard Nepal", className="display-4")),
        dbc.Col(
            html.Button(
...
sidebar = html.Div(
    [
        ...
        dbc.Collapse(
            dbc.Nav(
                [
                    dbc.NavLink("Clap me in Medium", href="https://medium.com/@dipesh.pandey42", target="_blank"),
                    dbc.NavLink("Messenger Chatbot", href="https://m.me/coronanepal2020", target="_blank"),
                ],
                vertical=True,
                pills=True,
            ),
...
content = dbc.Container(
    [
        ...
        dbc.Row(
            [
                dbc.Col(
                    dcc.Graph(id="out-33-dropdown", config=config, figure={}), md=12
                ),
            ],
            align="center",
        ),
            ...
            [
                dbc.Col(
                    [
                        ...
                        dcc.Graph(figure=agedistribution_list(), config=config),
                    ],
                    md=5,
                ),
                dbc.Col([dcc.Graph(id="out-42-dropdown", figure={}),], md=3,),
                dbc.Col(
                    [dcc.Graph(id="out-41-dropdown", config=config, figure={},)], md=4,
                ),
            ],
            align="center",
        ),
